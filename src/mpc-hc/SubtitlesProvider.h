...
virtual SRESULT Hash(SubtitlesInfo& pFileInfo);
virtual SRESULT Upload(const SubtitlesInfo& pSubtitlesInfo);
virtual std::string UserAgent() { return string_format("SubDB/1.0 (mpc-hc/%u.%u.%u; http://mpc-hc.org)", VersionInfo::GetMajorNumber(), VersionInfo::GetMinorNumber(), VersionInfo::GetPatchNumber()); }
DEFINE_SUBTITLESPROVIDER_END
...
DEFINE_SUBTITLESPROVIDER_BEGIN(Lua, "", IDI_YSUBS, SPF_SEARCH)
virtual void Initialize();
virtual void Uninitialize();
//lua::State state;
//lua::Value val;
