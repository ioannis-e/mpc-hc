...
string_map string_uncompress(const std::string& data, const std::string& fileName)
{
    ...
    static const unsigned char zlib[4][2] = { { 0x78, 0xDA }, { 0x78, 0x9C }, { 0x78, 0x5E }, { 0x78, 0x01 } };
    static const unsigned char zip[] = { 0x50, 0x4B, 0x03, 0x04 };
    static const unsigned char rar4[] = { 0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x00 };
    static const unsigned char rar5[] = { 0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x01, 0x00 };

    if (data.compare(0, sizeof(gzip), (const char*)gzip, sizeof(gzip)) == 0) {
        result.insert(std::pair<std::string, std::string>(fileName, string_gzinflate(data)));
    } else if ((data.compare(0, sizeof(zlib[0]), (const char*)zlib[0], sizeof(zlib[0])) == 0) || (data.compare(0, sizeof(zlib[1]), (const char*)zlib[1], sizeof(zlib[1])) == 0) ||
               (data.compare(0, sizeof(zlib[2]), (const char*)zlib[2], sizeof(zlib[2])) == 0) || (data.compare(0, sizeof(zlib[3]), (const char*)zlib[3], sizeof(zlib[3])) == 0)) {
        result.insert(std::pair<std::string, std::string>(fileName, string_gzuncompress(data)));
    } else if (data.compare(0, sizeof(zip), (const char*)zip, sizeof(zip)) == 0) {
        TCHAR path[MAX_PATH], file[MAX_PATH];
        GetTempPath(MAX_PATH, path);
        UINT unique = GetTempFileName(path, _T("mpc"), 0, file);
        UNREFERENCED_PARAMETER(unique);
        ...
        DeleteFile(file);
    } else if ((data.compare(0, sizeof(rar4), (const char*)rar4, sizeof(rar4)) == 0) || (data.compare(0, sizeof(rar5), (const char*)rar5, sizeof(rar5)) == 0)) {
        TCHAR path[MAX_PATH], file[MAX_PATH];
        GetTempPath(MAX_PATH, path);
        UINT unique = GetTempFileName(path, _T("mpc"), 0, file);
        UNREFERENCED_PARAMETER(unique);
...
HRESULT string_download(const std::string& url, const string_map& headers, std::string& data, BOOL bAutoRedirect, DWORD* dwStatusCode)
{
    ...
    try {
        ...
        CAutoPtr<CHttpFile> pHttpFile((CHttpFile*)is.OpenURL(UTF8To16(url.c_str()), 1, INTERNET_FLAG_TRANSFER_BINARY | INTERNET_FLAG_EXISTING_CONNECT | (bAutoRedirect == FALSE ? INTERNET_FLAG_NO_AUTO_REDIRECT : NULL), UTF8To16(strHeaders.c_str()), MAXDWORD));
