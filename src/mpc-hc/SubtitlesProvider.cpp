...
SRESULT OpenSubtitles::Login(const std::string& sUserName, const std::string& sPassword)
{
    if (xmlrpc) {
        ...
        bool exec = xmlrpc->execute("LogIn", args, result);
        TRACE(_T("%S::Login(UserAgent=%S, token=%S) = %S\n"), Name().c_str(), (const char*)args[3], result["token"].valid() ? (const char*)result["token"] : "", (const char*)result["status"]);

        if ((exec) && (result["token"].valid())) {
            token = result["token"];
            return SR_SUCCEEDED;
        }
    }
...
SRESULT OpenSubtitles::Search(const SubtitlesInfo& pFileInfo)
{
    if (token.valid()) {
        ...
        bool exec = xmlrpc->execute("SearchSubtitles", args, result);
        TRACE(_T("%S::Search(sublanguageid=%S, moviehash=%S, moviebytesize=%S) = %S\n"), Name().c_str(), (const char*)args[1][0]["sublanguageid"], (const char*)args[1][0]["moviehash"], (const char*)args[1][0]["moviebytesize"], (const char*)result["status"]);

        if ((exec) && (result["data"].getType() == XmlRpcValue::Type::TypeArray)) {
...
SRESULT OpenSubtitles::Download(SubtitlesInfo& pSubtitlesInfo)
{
    if (token.valid()) {
        ...
        bool exec = xmlrpc->execute("DownloadSubtitles", args, result);
        TRACE(_T("%S::Download(IDSubtitleFile=%S) = %S\n"), Name().c_str(), (const char*)args[1][0], (const char*)result["status"]);
...
std::string OpenSubtitles::Languages()
{
    ...
    if (data.empty() && CheckInternetConnection()) {
        XmlRpcValue args, result;
        args = "en";
        bool exec = xmlrpc->execute("GetSubLanguages", args, result);
...
SRESULT SubDB::Search(const SubtitlesInfo& pFileInfo)
{
    ...
    std::string url = string_format("http://api.thesubdb.com/?action=search&hash=%s", pFileInfo.fileHash.c_str());
...
{
    std::string url = string_format("http://api.thesubdb.com/?action=download&hash=%s&language=%s", pSubtitlesInfo.id.c_str(), pSubtitlesInfo.languageCode.c_str());
    TRACE(_T("%S::Download => %S\n"), Name().c_str(), url.c_str());
    return Download(url, "", pSubtitlesInfo.fileContents);
}
...
{
    std::string url = string_format("http://www.podnapisi.net/subtitles/%s/download", pSubtitlesInfo.id);
    TRACE(_T("%S::Download => %S\n"), Name().c_str(), url.c_str());
...
SRESULT ysubs::Search(const SubtitlesInfo& pFileInfo)
{
    ...
    if (pFileInfo.year && pFileInfo.seasonNumber == -1 && pFileInfo.episodeNumber == -1) {
        std::string url(string_format("https://yts.to/api/v2/list_movies.json?query_term=%s", UrlEncode(pFileInfo.title.c_str())));
        TRACE(_T("%S::Search => %S\n"), Name().c_str(), url.c_str());
...
void Lua::Initialize()
{
//     auto Register = [&](lua::Value t) {
//         val = t;
//         ref = val;
//     };
//
//     state.set("Register", Register);
//
//     try {
//         val=state.doFile("P:\\test1.lua");
//         if ((val)["Name"]) {
//             std::string name;
...
//         if ((val)["Url"]) {
//             std::string name;
//             (val)["Url"]().getString(name);
//             name.clear();
//         }
//         if ((val)["UserAgent"]) {
//             std::string name;
//             (val)["UserAgent"]().getString(name);
//             name.clear();
//         }
...
SRESULT Lua::Search(const SubtitlesInfo& pFileInfo)
{
    SRESULT searchResult = SR_UNDEFINED;
//     try {
//         if ((val)["Name"]) {
...
SRESULT Lua::Download(SubtitlesInfo& pSubtitlesInfo)
{
    SRESULT searchResult = SR_UNDEFINED;
//     try {
//         if ((val)["Download"]) {
...
{
    std::string url = string_format("http://www.tvsubtitles.net/download-%s.html", pSubtitlesInfo.id.c_str());
    TRACE(_T("%S::Download => %S\n"), Name().c_str(), url.c_str());
...
{
    std::string url = string_format("http://www.moviesubtitles.org/download-%s.html", pSubtitlesInfo.id.c_str());
    TRACE(_T("%S::Download => %S\n"), Name().c_str(), url.c_str());
...
SRESULT addic7ed::Search(const SubtitlesInfo& pFileInfo)
{
    ...
    if (pFileInfo.seasonNumber != -1) {
        ...
        std::string url = string_format("http://www.addic7ed.com/search.php?search=%s", UrlEncode(search.c_str()));
        TRACE(_T("%S::Search => %S\n"), Name().c_str(), url.c_str());
